...
// TODO Rename start to source
    public IEnumerable<Line> AStar(Point source,
        Point target,
        Func<Point, Point, double> costHeuristic)
    {
        ...
        while (openSet.Count > 0)
        {
            ...
            foreach (var neighbour in _adjacentList[current])
            {
                ...
                var tentativeGScore = (gScores.ContainsKey(current) ? gScores[current] : 0)
                    + Point.DistanceBetween(current, neighbour);
                ...
                fScores[neighbour] = gScores[neighbour] + costHeuristic(neighbour, target);
            }
        }
